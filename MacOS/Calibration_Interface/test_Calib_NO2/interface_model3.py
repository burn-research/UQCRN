...
data_sensor = pd.read_csv('SensorDataNO2_R8.txt', sep = ',', header=None)
...
for T in T_cstr_1:
    for phi in phi_rich:
        ...
        # Calibration only on one reactor
        outname = 'Output/Reactor.8/Output.out'
        dt = pd.read_csv(outname, sep = '\s+')
        output_values_global.append(dt['NO2_w(61)'].values[-1]*1e6)
        output_names_global.append('NO2_Case_' + str(intcounter)); intcounter+=1

print('All the CRN values:')
print(output_values_global)
print('All the CFD values:')
print(np.array(data_sensor.values).T.reshape(-1,1)*1e6)
SSE = np.sum((np.array(output_values_global).reshape(-1,1) - np.array(data_sensor.values).T.reshape(-1,1)*1e6)**2)

# Write outputs to file
output_dakota = sys.argv[2]
with open(output_dakota, 'w') as f:
    f.write(str(SSE))
#    for i in range(len(output_values_global)):
#        f.write(str(output_values_global[i]) + ' ' + output_names_global[i] + '\n')
